      ...
      {content.children.map((node: any, index: number) => {
          ...
          return (
            <p
              ...
              dangerouslySetInnerHTML={{
                __html: renderChildren(node.children),
              }}
            />
          );
        }
          ...
          return React.createElement(
            ...
            node.children.map((item: any, itemIndex: number) => (
              <li
                key={itemIndex}
                dangerouslySetInnerHTML={{
                  __html: renderChildren(item.children),
                }}
              />
            ))
          );
        ...
        if (node.type === 'horizontalrule') {
          return (
            <hr
              key={index}
              className="my-6 border-neutral-700"
            />
          );
        }
        ...
        if (node.type === 'block' && node.fields?.blockType === 'conclusion') {
          return (
            <div
              key={index}
              className="mt-8 p-6 bg-primary-500/10 border border-primary-500/20 rounded-xl"
            >
              ...
              {node.fields.keyTakeaways?.length > 0 && (
                <div className="mb-4">
                  ...
                  {node.fields.keyTakeaways.map(
                    (takeaway: any, takeawayIndex: number) => (
                      <p
                        key={takeawayIndex}
              ...
              {node.fields.nextSteps?.enabled &&
                node.fields.nextSteps.actions?.length > 0 && (
                  <div>
                    <h4 className="text-sm font-medium text-neutral-400 mb-2">
                      {node.fields.nextSteps.title}
                    </h4>
                    {node.fields.nextSteps.actions.map(
