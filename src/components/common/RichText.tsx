...
const RichText: React.FC<RichTextProps> = ({ content, className }) => {
    ...
    return children.map((node, i) => {
      ...
      if (node.type === 'text' || node.text !== undefined) {
        ...
        if (node.format) {
          ...
          if (node.format & 4)
            text = <s className="line-through opacity-75">{text}</s>;
          if (node.format & 8)
            text = (
          ...
          if (node.format & 16)
            text = (
              <code className="bg-muted/50 dark:bg-muted/30 px-2 py-1 rounded-md text-sm font-mono border">
                {text}
              </code>
            );
        }
      ...
      switch (node.type) {
        ...
        case 'list':
          const ListTag =
            node.listType === 'number' || node.listType === 'numbered'
              ? 'ol'
              : 'ul';
          ...
          return (
            <ListTag
              ...
              className={cn(
                ...
                indent > 0 &&
                  `ml-${Math.min(6 + indent * 4, 20)} md:ml-${Math.min(8 + indent * 6, 24)}`
              )}
              role="list"
            >
              {children}
        ...
        case 'link':
          ...
          return (
            ...
            >
              ...
              {isExternal && (
                <span
                  className="ml-1 -space-y-1 text-xs"
                  aria-hidden="true"
                >
                  ↗
                </span>
        ...
        case 'upload':
          ...
          const imageUrl =
            imageData.url || `/api/media/file/${imageData.filename}`;
          ...
          return (
            ...
            >
              <img
                ...
                width={imageData.sizes.card.width || node.width}
                height={imageData.sizes.card.height || node.height}
                loading="lazy"
                fetchPriority="auto"
                decoding="auto"
              />
        ...
        case 'relationship':
          ...
          if (node.value && typeof node.value === 'object') {
            return (
              <div
                key={i}
                className="max-w-4xl my-6 p-4 bg-gradient-to-r from-accent to-primary/10 border border-primary/20 rounded-lg"
              >
        ...
        case 'table':
          return (
            <div
              key={i}
              className="my-6 md:my-8 overflow-x-auto rounded-lg border border-border shadow-sm"
            >
              <table
          ...
          return (
            <CellTag
              ...
              className={cn(
                ...
                node.headerCell &&
                  'bg-muted/50 font-ui font-semibold text-foreground border-b-2 border-border'
              )}
  ...
  const contentToRender =
    content?.root?.children || content?.children || content || [];
