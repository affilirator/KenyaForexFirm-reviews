...
import { extractDomain, normalizeRegulatorName, areRegulatorNamesSimilar } from '../utils/regulatorUtils';
 ...
 */
export async function getBrokersByRegulatorWebsite(website: string, limit: number = 5): Promise<BrokerReviewsResponse> {
  try {
      ...
      docs: allBrokers.docs
        .filter(broker => {
          ...
          if (broker.regulators && broker.regulators.length > 0) {
            return broker.regulators.some(reg => 
              typeof reg === 'string' && 
    ...
    console.error(`Failed to fetch brokers by regulator website (${website}):`, error);
    throw error;
  }
}
 ...
 * @returns Promise with broker reviews response
 */
export async function getBrokersByRegulatorName(regulatorName: string, limit: number = 5): Promise<BrokerReviewsResponse> {
  try {
    // Build query parameters
    const params = new URLSearchParams({
      'limit': '20', // Fetch more to filter
    ...
    console.error(`Failed to fetch brokers by regulator name (${regulatorName}):`, error);
    throw error;
  }
}