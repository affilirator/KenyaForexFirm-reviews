...
const brokers = await (await fetch('https://api.kenyaforexfirm.com/api/broker-reviews?limit=1000')).json() as BrokerReviewsResponse;
export const regulatedBrokers = brokers.docs.filter(broker => broker.isRegulated || broker.cmaRegulated || (broker.regulators && broker.regulators.length > 0));
...
export const allRegulators = regulatedBrokers.flatMap(broker => broker.regulators || []);
