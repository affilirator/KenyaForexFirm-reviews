...
export const getRegulatorBySlug = async (slug: string): Promise<Regulator | null> => {
  try {
    const response = await fetch(`${PAYLOAD_API_URL}/api/regulators?where[slug][equals]=${slug}`);
