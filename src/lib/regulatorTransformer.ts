...
export function transformRegulatorData(regulator: Regulator): RegulatorPage {
  ...
  if (regulator.brokerRequirements && regulator.brokerRequirements.length > 0) {
    sections.push({
      ...
      content: textToRichText(
        'Requirements for brokers regulated by ' + regulator.name
      ),
  ...
  if (regulator.investorProtection && regulator.investorProtection.length > 0) {
    sections.push({
      ...
      content: textToRichText(
        'Protections offered to investors by ' + regulator.name
      ),
  ...
  const faqSection =
    regulator.faqs && regulator.faqs.length > 0
      ? {
          sectionTitle: 'Frequently Asked Questions',
          sectionId: 'faq',
          introduction: textToRichText(
            'Common questions about ' + regulator.name
          ),
  ...
  const conclusionSection = {
    ...
    content: textToRichText(
      `${regulator.name} (${regulator.shortName}) is a regulatory authority based in ${regulator.country}. Established in ${regulator.established}, it oversees financial markets and provides oversight for forex brokers operating in the region.`
    ),
  };
